...
func (m Model) renderHelpPage1() string {
	...
	right.WriteString(fmt.Sprintf("  %s  Open editor (Caddy)\n", StyleKeybinding.Render("E")))
...
func (m Model) renderHelpPage4() string {
	...
	left.WriteString(fmt.Sprintf("  %s  Open editor (Caddy)\n", StyleKeybinding.Render("E")))
	left.WriteString(fmt.Sprintf("  %s  Migrate Caddyfile\n", StyleKeybinding.Render("m")))
	left.WriteString(fmt.Sprintf("  %s  Refresh data\n", StyleKeybinding.Render("r")))

	var right strings.Builder
	right.WriteString(StyleInfo.Render("Profile Selector"))
	right.WriteString("\n")
	right.WriteString(fmt.Sprintf("  %s  Edit profile\n", StyleKeybinding.Render("e")))
	right.WriteString(fmt.Sprintf("  %s  Delete profile\n", StyleKeybinding.Render("d")))
	right.WriteString(fmt.Sprintf("  %s  Export profile\n", StyleKeybinding.Render("x")))
	right.WriteString(fmt.Sprintf("  %s  Import profile\n", StyleKeybinding.Render("i")))
	right.WriteString(fmt.Sprintf("  %s  New profile\n", StyleKeybinding.Render("+")))
	right.WriteString("\n")
	right.WriteString(StyleInfo.Render("Backup Manager"))
	right.WriteString("\n")
	right.WriteString(fmt.Sprintf("  %s  Preview/Restore\n", StyleKeybinding.Render("Enter/R")))
	right.WriteString(fmt.Sprintf("  %s  Delete / Cleanup\n", StyleKeybinding.Render("x / c")))
...
func (m Model) renderHelpPage5() string {
	...
	var right strings.Builder
	right.WriteString(StyleInfo.Render("Profile Edit"))
	right.WriteString("\n")
	right.WriteString(fmt.Sprintf("  %s  Navigate fields\n", StyleKeybinding.Render("↑/↓/Tab")))
	right.WriteString(fmt.Sprintf("  %s  Edit field\n", StyleKeybinding.Render("Enter")))
	right.WriteString(fmt.Sprintf("  %s  Toggle checkbox\n", StyleKeybinding.Render("Space")))
	right.WriteString(fmt.Sprintf("  %s  Save profile\n", StyleKeybinding.Render("Ctrl+S")))
	right.WriteString(fmt.Sprintf("  %s  Cancel\n", StyleKeybinding.Render("Esc")))
	right.WriteString("\n")
	right.WriteString(StyleInfo.Render("Audit Log"))
	right.WriteString("\n")
	right.WriteString(fmt.Sprintf("  %s  Search\n", StyleKeybinding.Render("/")))
	right.WriteString(fmt.Sprintf("  %s  Filter op / result\n", StyleKeybinding.Render("f / r")))
